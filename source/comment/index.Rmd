...
During the 2016 survey, all 60 churches of the 1980s survey have been revisted and 50 additional churches. The authors take the repeated measures into account by using a McNemar change test. This test works only on 2x2 tables. 

A better alternative would be to use cumulative logit mixed models[@ordinal]. In this case we would use survey and light status

```{r}
status_model <- clmm(Status ~ Survey + Lights + (1|Church), data = status)
...
cumulative_prediction <- cumulative_logit %>%
  ...
  as.data.frame() %>%
  rownames_to_column("Contrast") %>%
  extract(Contrast, c("Status", "Survey", "Lights"), "(.*):(.*):(.*)")
```
