...
abstract: |
  ...
  We have no intent to discrit the origal authors. Their analysis was basic but correct.
...
renovation_model %>%
  ...
  kable(
    digits = c(3, 3, 2, 4),
    caption = "Coefficients of the cumulative logit model for the renovation model."
  )
```
...
We find no evidence for an effect between surveys `r renovation_or$print[3]` (LTR: `r renovation_lrt[1]`). The light status has a significant effect (LTR: `r renovation_lrt[2]`). We find an unclear effect of partly lit vs dark `r renovation_or$print[4]`, but a the odd for no bats between fully lit and dark increases `r renovation_or$print[5]`. Addionally after renovation, the odds for no bat increase `r renovation_or$print[6]` (LTR: `r renovation_lrt[3]`). The resulting predictions are given in fig. \@ref{fig:renovation-prediction}. We could wonder which is worse: a fully lit, non renovated church or a dark, renovated church. 
...
```{r renovation-prediction, fig.cap = "Estimated cumulative probability"}
ggplot(
  cumulative_prediction, 
  aes(
    x = Lights, 
